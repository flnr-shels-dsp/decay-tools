...
df_selected = read_and_select_columns("SECH.CSV")
display(df_selected)
...
df_clean = cluster_by_energy(df_selected, n_groups=5)
...
check_grouping_completeness(df_clean)
...
def plot_energy_groups(df_clean: pd.DataFrame, n_groups: int) -> None:
    ...
    plt.tight_layout()
    plt.show()
plot_energy_groups(df_clean, n_groups=5)
...
summary_df = calculate_group_summary(df_clean, n_groups=5)

def sum_integral_and_ion_count(ion_charge: float, n_groups: int) -> pd.DataFrame:
...
n_groups = df_clean["EnergyGroup"].nunique()

plot_cross_section_with_energy_errors(df_clean, summary_df, sigma_df, n_groups)
...
final_table = build_cross_section_table(df_clean, summary_df, sigma_df, n_groups)
